...
module.exports = function (apk, callback) {
  ...
  var apk = path.join(__dirname, '../../')+apk;
  ...
  parseApk(apk, function (err, data) {
    ...
    if (!packageName) {
      return callback(false, "Failed to load packageName from APK");
    ...
    function createExtension(packageName) {
      ...
      var appPath = path.join('extensions/' + packageName + '.android');
      ...
      ncp(templatePath, appPath, function (err) {
        ...
        if(typeof messages?.extName?.message === 'undefined' || messages?.extName?.message === null || messages?.extName?.message === '') {
          messages.extName.message = "app.generated.raid2";
        }
